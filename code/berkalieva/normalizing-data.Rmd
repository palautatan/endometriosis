...
``` {r}
# normalize the data
normalized <- normalizeBetweenArrays(probeset, method = "quantile")
normalize_loess <- normalizeBetweenArrays(probeset, method = "cyclicloess")

# transform to data frame for easy plotting
non_normalized <- as.data.frame(probeset)
normalized <- as.data.frame(normalized)
normalized_loess <- as.data.frame(normalize_loess)

# create boxplots (for ALL samples)
par(mfrow=c(1,3))
